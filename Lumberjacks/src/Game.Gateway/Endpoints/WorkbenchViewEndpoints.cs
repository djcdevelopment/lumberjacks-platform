...
/// <summary>
/// Serves the generated, self-contained Community Workbench catalog. The source of truth is
/// docs/workbench/workbench.json; scripts/workbench.mjs renders this published asset and checks
/// that it is neither stale nor script-bearing. The document is re-read per request behind an
/// mtime-and-length cache, and <c>LUMBERJACKS_WORKBENCH_HTML</c> relocates it, so a freshly
/// rendered catalog can be mounted over the built-in copy and republished without an image
/// rebuild or a restart. Resolution degrades in steps — mounted asset, then the copy built into
/// the image, then the fallback page — so a missing mount costs freshness, not the page. Every
/// response carries the SHA-256 of the file on disk in <c>X-Workbench-Sha256</c>, so a served
/// page can be verified byte-for-byte against the committed artifact. Like the other community
/// surfaces, a missing optional asset degrades to a small explanation instead of preventing the
/// Gateway from starting.
/// </summary>
public static class WorkbenchViewEndpoints
{
    // Two segments, not "Community/workbench.html": Path.Combine keeps an embedded forward
    // slash verbatim, so the one-string form resolved to a path that only matched a
    // Path.Combine-built expectation on Linux.
    private const string AssetDirectory = "Community";
    private const string AssetFile = "workbench.html";
    private const string PathVariable = "LUMBERJACKS_WORKBENCH_HTML";
    private const string ShaHeader = "X-Workbench-Sha256";

    private const string FallbackHtml =
        "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><title>Lumberjacks — Community Workbench</title></head>" +
        "<body style=\"background:#0b1013;color:#edf5f2;font-family:system-ui,sans-serif;padding:2rem\">" +
        "<h1>Community Workbench unavailable</h1>" +
        "<p>The generated workbench asset was not included in this Gateway build. " +
        "Run <code>npm run workbench:render</code>, publish again, or read " +
        "<code>docs/workbench/workbench.json</code> directly.</p>" +
        "</body></html>";

    /// <summary>A resolved workbench document plus the stat fields that decide whether it is stale.</summary>
    ...
    public static void Map(WebApplication app)
    {
        ...
        app.MapGet("/workbench", (HttpContext context) =>
            {
    ...
    private static Document Load(string contentRoot, ILogger logger)
    {
        ...
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            logger.LogWarning(ex,
                "Could not load {Path} for GET /workbench — serving a minimal fallback page instead.",
                path);
