...
.DESCRIPTION
Copies each platform-owned -Path item (file or directory, recursively) into
-Dest on the i5. Default destination is C:/deploy/lumberjacks-platform. After the copy,
...
i5. OpenSSH >= 9.0 client (scp in SFTP mode -- remote
...
Default: C:/deploy/lumberjacks-platform (the staging root -- auto-created).
...
.EXAMPLE
.\Deploy-ToI5.ps1 -Path .\bundle\ -Dest C:/deploy/lumberjacks-platform/run-042
#>
[CmdletBinding()]
param(
    ...
    [string]$Dest = 'C:/deploy/lumberjacks-platform',
...
if ($LASTEXITCODE -ne 0) {
    throw 'i5 ssh lane is offline (normal for this roaming laptop); no password fallback was attempted'
}
