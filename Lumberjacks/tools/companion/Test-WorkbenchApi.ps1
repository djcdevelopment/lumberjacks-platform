...
Assert-That ($projection.live.schema_version -eq 1) 'live operator projection schema missing'
Assert-That ($projection.live.event_type -eq 'workbench.live_status') 'live operator event type missing'
Assert-That (-not [string]::IsNullOrWhiteSpace($projection.live.headline)) 'live operator headline missing'
Assert-That (-not [string]::IsNullOrWhiteSpace($projection.live.next_action)) 'live operator next action missing'
Assert-That ($null -ne $projection.live.server.PSObject.Properties['peer_count']) 'live player-presence projection missing'
Assert-That ($null -ne $projection.live.activity.PSObject.Properties['executing']) 'live machine-activity distinction missing'
