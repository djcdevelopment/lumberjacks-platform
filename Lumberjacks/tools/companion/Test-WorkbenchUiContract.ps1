...
Assert-That ($html -match 'Live operation') 'live operation summary missing from Workbench Home'
Assert-That ($html -match "Who's online") 'live player-presence card missing from Workbench Home'
Assert-That ($html -match 'Current activity') 'live activity card missing from Workbench Home'
Assert-That ($html -match 'Runtime netcode') 'live cutover/motion card missing from Workbench Home'
Assert-That ($html -match "live-headline'\)\.textContent=l\.headline") 'live status projection is not rendered into the operator headline'
Assert-That ($html -match "live-next'\)\.textContent=l\.next_action") 'live recommended next action is not rendered'
Assert-That ($html -match '/companion.*detailed gameplay telemetry') 'existing telemetry-rich Companion surface is not linked from Home'
