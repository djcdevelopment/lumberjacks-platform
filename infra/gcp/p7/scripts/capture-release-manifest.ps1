...
param(
  ...
  [string] $ManifestPath = (Join-Path ([IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..\..\..\..'))) 'captures\releases\p7-primary-v1-20260715.json'),
...
$sourceRevision = $ModSourceRevision.Trim()
if ($sourceRevision -notmatch '^[0-9a-f]{40}$') { throw 'ModSourceRevision must be a full commit SHA.' }
