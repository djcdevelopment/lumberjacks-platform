...
# quest-picker.html is deliberately NOT in this publish set. The served /questpicker page is
# baseline's data projection — the guild catalogs live in that repo, behind a generator pin that
# authorizes each refresh — and baseline's tools/questpicker/Publish-QuestPickerToAM4.ps1 is the
# only writer of $RemoteRoot/quest-picker.html on the box. Community/quest-picker.html here is
# the SAMPLE, image-baked as the cold-start fallback; shipping it from this script is how the
# live surface served "Sample Guild" from 2026-08-12 to 2026-08-17 while the real 314-quest page
# sat committed in baseline. verify-live keeps /questpicker as a nav-route liveness check, which
# holds no matter who published the content.
